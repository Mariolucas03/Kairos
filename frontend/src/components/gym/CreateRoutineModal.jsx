...
import { X, Plus, Save, Trash2, Dumbbell, ArrowUp, ArrowDown, Check, Timer, Hash, Play, Clock, PersonStanding, MoveHorizontal, Link2, TrendingUp, SlidersHorizontal } from 'lucide-react';
...
export default function CreateRoutineModal({ onClose, onRoutineCreated, routineToEdit = null }) {
    ...
    // Qué ejercicio tiene los ajustes desplegados. Solo uno a la vez: la lista
    // de ejercicios es lo que más se mira al montar una rutina, y llenarla de
    // interruptores la vuelve ilegible. Los ajustes finos se piden pocas veces
    // y por eso van escondidos detrás de un botón.
    const [ajustesAbiertos, setAjustesAbiertos] = useState(null);
                            ...
                            addedExercises.map((ex, idx) => (
                                <div key={idx} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 group animate-in slide-in-from-bottom-2 duration-300">
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-4 flex-1 min-w-0">
                                        ...
                                        <div className="min-w-0 pr-2">
                                            ...
                                            {/* Resumen de lo que está configurado. Sin esto habría
                                                que abrir cada ejercicio para saber cuál tiene qué. */}
                                            {(ex.esPorTiempo || ex.esPesoCorporal || ex.porLado || ex.superserie || (ex.progresion && ex.progresion !== 'ninguna')) && (
                                                <p className="text-[9px] text-yellow-500/80 font-bold uppercase tracking-wide mt-1 truncate">
                                                    {[
                                                        ex.esPorTiempo && 'Tiempo',
                                                        ex.esPesoCorporal && 'Corporal',
                                                        ex.porLado && 'Por lado',
                                                        ex.superserie && `Serie ${ex.superserie}`,
                                                        ex.progresion && ex.progresion !== 'ninguna' && `${PROGRESIONES.find(p => p.valor === ex.progresion)?.texto} +${ex.incremento ?? 2.5}kg`
                                                    ].filter(Boolean).join(' · ')}
                                                </p>
                                            )}
                                        </div>
                                  ...
                                  >
                                    ...
                                    {ajustesAbiertos === idx ? 'Cerrar ajustes' : 'Cómo se mide y progresión'}
                                  </button>
                                  ...
                                  {ajustesAbiertos === idx && (
                                    ...
                                    <div>
                                        <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest mb-2">Qué peso toca la próxima vez</p>
                                        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-black border border-zinc-800">
                                            <TrendingUp size={12} className={ex.progresion && ex.progresion !== 'ninguna' ? 'text-yellow-500' : 'text-zinc-600'} />
                                            <select
