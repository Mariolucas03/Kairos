...
export default function Register() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (loading) return;
        if (!formData.gender) { setError('Elige hombre o mujer: es el cuerpo que se va a pintar en el mapa muscular.'); return; }
        setLoading(true);
        setError(null);
        ...
        <PantallaAuth
            ...
            pie={
                <p className="text-[12px] text-zinc-500 font-medium not-italic">
                    ¿Ya tienes cuenta?{' '}
                    <Link to="/login" className="font-black hover:brightness-125 transition-all" style={{ color: ACENTO }}>
                        Entra
                    </Link>
                </p>
            }
        >
            <form onSubmit={handleSubmit} className="space-y-4">
                <CampoAuth
                    ...
                    contador={
                        <span
                            className="text-[10px] font-bold not-italic transition-colors"
                            style={{ color: letrasRestantes === 0 ? ACENTO : '#52525b' }}
                        >
                            ...
                            <div className="flex gap-1 flex-1">
                                {[1, 2, 3, 4, 5].map(n => (
                                    <span
                                        ...
                                        style={{ background: n <= fuerza.nivel ? fuerza.color : '#27272a' }}
                                    />
                                ))}
                            </div>
                            <span
                                className="text-[10px] font-bold not-italic w-[68px] text-right transition-colors"
                ...
                {/* Hombre o mujer: decide que cuerpo se pinta en el mapa muscular */}
                <div>
                    <label className="block text-[11px] font-bold text-zinc-400 mb-1.5 not-italic">Cuerpo</label>
                        ...
                        {[['male', 'Hombre'], ['female', 'Mujer']].map(([valor, nombre]) => {
                            ...
                            return (
                                <button
                                    ...
                                    className="h-12 rounded-xl border text-[13px] font-bold transition-colors not-italic"
                                    style={{
                                        borderColor: elegido ? ACENTO : 'rgba(255,255,255,0.1)',
                                        background: elegido ? ACENTO + '1f' : '#0f0f11',
                                        color: elegido ? '#fff' : '#a1a1aa'
                                    }}
                                >
                                    {nombre}
                                </button>
                ...
                <BotonAuth cargando={loading} acento={ACENTO} textoCargando="Creando…" colorTexto="#fff">
                    Crear cuenta
                </BotonAuth>
            </form>
        </PantallaAuth>
