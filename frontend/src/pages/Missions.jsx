        ...
        <div className="relative w-full mb-4 select-none group" onClick={() => viewAllMode && onEdit(mission)}>
            ...
            >
                ...
                <div className="p-4 relative overflow-hidden h-full flex flex-col justify-between">
                    ...
                    <div className="relative z-10">
                            ...
                            {mission.frequency === 'daily' && mission.specificDays?.length > 0 && (
                                <span className="flex items-center gap-1 px-1.5 py-1 rounded-xl border border-white/[0.07] bg-white/[0.03]">
                                    {[1, 2, 3, 4, 5, 6, 0].map(d => (
                                        <span
                                            key={d}
                                            className={`w-[13px] h-[13px] rounded-full flex items-center justify-center text-[9px] font-black not-italic ${mission.specificDays.includes(d) ? 'bg-zinc-600 text-white' : 'text-zinc-600'}`}
                                        >
                                            {DAY_LABELS[d]}
                                        </span>
                                    ))}
                        ...
                        {!isBinary && !mission.completed && !isPending && !viewAllMode && (
                            ...
                            >
                                ...
                                <input
                                    ...
                                    className="flex-1 min-w-0 h-9 bg-black border border-zinc-800 rounded-xl px-2 text-white font-black text-sm text-center outline-none focus:border-zinc-600 transition-all placeholder:text-zinc-500 placeholder:font-bold"
                                    value={inputValue}
                            ...
                            <div className="flex items-center gap-3 mt-2">
                                {mission.participants.map(p => (
                                    <div key={p._id} className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-[0.08em] truncate max-w-[80px] not-italic">
                                            {p.username}
                                        </span>
                                    </div>
                                ))}
                        ...
                        <div className="flex gap-4 mt-3 pt-3 border-t border-white/[0.07] relative z-10 items-center">
                            <div className="flex items-center gap-3">
                                {mission.xpReward > 0 && (
                                    <div className="flex items-center gap-1"><span className={`text-[12px] font-black not-italic ${mission.completed ? 'text-zinc-500' : 'text-zinc-300'}`}>+{mission.xpReward}</span><img src={ICON_XP} className={`w-5 h-5 object-contain ${mission.completed ? 'grayscale opacity-50' : ''}`} alt="XP" /></div>
                                )}
                                {(mission.coinReward > 0) && (
                                    <div className="flex items-center gap-1"><span className={`text-[12px] font-black not-italic ${mission.completed ? 'text-zinc-500' : 'text-zinc-300'}`}>+{mission.coinReward}</span><img src={ICON_COIN} className={`w-5 h-5 object-contain ${mission.completed ? 'grayscale opacity-50' : ''}`} alt="Monedas" /></div>
                                )}
                                {(mission.gameCoinReward > 0) && (
                                    <div className="flex items-center gap-1"><span className={`text-[12px] font-black not-italic ${mission.completed ? 'text-zinc-500' : 'text-zinc-300'}`}>+{mission.gameCoinReward}</span><img src={ICON_CHIP} className={`w-5 h-5 object-contain ${mission.completed ? 'grayscale opacity-50' : ''}`} alt="Fichas" /></div>
                                )}
                            </div>
                    ...
                    {isPending && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 rounded-2xl z-30">
    ...
    return (
        <div className="min-h-screen bg-black text-white pb-24 animate-in fade-in relative">
                ...
                {filteredMissions.length === 0 ? (
                    <div className="py-20 text-center">
                        <div className="w-16 h-16 bg-[#0a0a0c] border border-white/[0.07] rounded-full flex items-center justify-center mx-auto mb-4">
                            <Target className="text-zinc-600" size={30} />
                        </div>
                    ...
                    <>
                        ...
                        {!viewAllMode && (
                            <div className="text-center mt-6 mb-4">
                                <span className="text-[9px] font-black text-zinc-500 uppercase tracking-[0.08em] flex items-center justify-center gap-1.5 not-italic">
            ...
            {showEditModal && missionToEdit && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/95 animate-in fade-in">
            ...
            {showCreator && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-0 sm:p-4 bg-black/95 animate-in fade-in duration-200">
